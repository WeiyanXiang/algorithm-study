...
public class StreamPracticer {
    public static void main(String[] args) {
        ...
        orders.stream().filter(o -> o.getOrderDate().isAfter(LocalDate.of(2021, 1, 31)) && o.getOrderDate().isBefore(LocalDate.of(2021, 3, 1)))
                .flatMap(it -> it.getProducts().stream()).mapToDouble(Product::getPrice).sum();
        ...
        orders.stream().filter(o -> o.getOrderDate().isEqual(LocalDate.of(2021, 3, 14)))
                .flatMap(o -> o.getProducts().stream()).mapToDouble(Product::getPrice).average();
        ...
        Map<Long, Integer> orderIdWithCountOfProductsAnother = orders.stream()
                .collect(toMap(Order::getId, p -> p.getProducts().size()));
        // Exercise 12 — Produce a data map with order records grouped by customer
        Map<Customer, List<Order>> collect1 = orders.stream().collect(groupingBy(Order::getCustomer));
        // Exercise 13 — Produce a data map with order record and product total sum
        Map<Order, Double> collect2 = orders.stream()
                .collect(toMap(Function.identity(), o -> o.getProducts().stream().mapToDouble(Product::getPrice).sum()));
        Map<Order, Double> collect3 = orders.stream()
                .collect(toMap(it -> it, o -> o.getProducts().stream().collect(summingDouble(Product::getPrice))));
