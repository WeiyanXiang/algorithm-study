...
public class practiceManager {
    public static void main(String[] args) {
        ...
        products.stream().filter(p -> "book".equalsIgnoreCase(p.getCategory()) && p.getPrice() > 100).collect(Collectors.toList());
        // Exercise 2 — Obtain a list of order with products belong to category “Baby”
        orders.stream().filter(o -> o.products.stream().anyMatch(p -> "baby".equalsIgnoreCase(p.getCategory()))).collect(Collectors.toList());
        // Exercise 3 — Obtain a list of product with category = “Toys” and then apply 10% discount
        products.stream().filter(p -> p.getCategory().equalsIgnoreCase("Toys")).map(p -> {
        ...
        products.stream().filter(p -> p.getCategory().equalsIgnoreCase("Books")).collect(Collectors.minBy(Comparator.comparingDouble(p -> p.getPrice())));
        products.stream().filter(p -> p.getCategory().equalsIgnoreCase("Books")).min(Comparator.comparingDouble(p -> p.getPrice())).orElse(null);
        // Exercise 6 — Get the 3 most recent placed order
        List<Order> recentOrders = orders.stream().sorted(Comparator.comparing(Order::getOrderDate).reversed()).limit(3).collect(Collectors.toList());
        ...
        List<Product> productList = orders.stream().filter(o -> o.getOrderDate().isEqual(LocalDate.of(2021, 3, 15)))
                .peek(System.out::println).flatMap(o -> o.getProducts().stream()).distinct().collect(Collectors.toList());
        ...
        orders.stream().filter(o -> o.getOrderDate().isAfter(LocalDate.of(2021, 1, 31)) && o.getOrderDate().isBefore(LocalDate.of(2021, 3, 1)))
                .flatMap(o -> o.getProducts().stream()).map(Product::getPrice).mapToDouble(e -> e).sum();
        ...
        Double collect = orders.stream().filter(o -> o.getOrderDate().isEqual(LocalDate.of(2021, 3, 14)))
                .flatMap(o -> o.getProducts().stream()).collect(Collectors.averagingDouble(p -> p.getPrice()));
        // Exercise 10 — Obtain a collection of statistic figures (i.e. sum, average, max, min, count) for all products of category “Books”
        products.stream().filter(p -> p.getCategory().equalsIgnoreCase("Books")).collect(Collectors.summarizingDouble(Product::getPrice));
